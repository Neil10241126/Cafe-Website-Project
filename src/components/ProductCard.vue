<template>
  <div class="card border border-gray-400 rounded-3" style="max-width: 300px;">
    <!-- 圖片 URL -->
    <img :src="img_url" class="card-img-top rounded-top-3"
      alt="Honduran-coffee-bean-2">
     <!-- 收藏按鈕 -->
    ...
    <div class="card-body bg-secondary-tint rounded-bottom-3 px-2 p-xxs-3">
      ...
      <div v-if="card_details" class="mb-2">
        ...
        <div class="d-flex align-items-center">
          <span class="fs-6 text-primary me-2"><strong>{{ `售價$ ${price}` }}</strong></span>
          <span class="text-gray-600"><del>{{ `原價$ ${origin_price}` }}</del></span>
        </div>
      </div>
